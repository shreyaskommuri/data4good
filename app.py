...
from src.models.resilience_ode import (
    ...
    optimize_policy_allocation,
)

from src.models.rapidfire import (
    generate_board_agenda_letter,
    generate_public_comment,
)
...
# ============================================================================
# SB 1000 EJ COMMUNITIES & SUPERVISORS
# ============================================================================

SB_EJ_COMMUNITIES = {
    'Isla Vista':   {'tracts': ['06083002802', '06083002801'], 'district': 3, 'lat': 34.4133, 'lon': -119.8567},
    'Guadalupe':    {'tracts': ['06083002600'],                'district': 4, 'lat': 34.9714, 'lon': -120.5717},
    'Casmalia':     {'tracts': ['06083002500'],                'district': 4, 'lat': 34.8338, 'lon': -120.5304},
    'Cuyama':       {'tracts': ['06083002400'],                'district': 5, 'lat': 34.9360, 'lon': -119.6350},
    'New Cuyama':   {'tracts': ['06083002400'],                'district': 5, 'lat': 34.9503, 'lon': -119.6927},
    'Ventucopa':    {'tracts': ['06083002400'],                'district': 5, 'lat': 34.8861, 'lon': -119.4597},
    'Garey':        {'tracts': ['06083002500'],                'district': 4, 'lat': 34.9486, 'lon': -120.4128},
    'Sisquoc':      {'tracts': ['06083002500'],                'district': 4, 'lat': 34.8722, 'lon': -120.2806},
}
...
TRACT_TO_DISTRICT = {
    '060830015': 2, '060830016': 2, '060830017': 2, '060830018': 2,
    '060830019': 2, '060830020': 1, '060830021': 1, '060830022': 1,
    '060830023': 3, '060830024': 5, '060830025': 4, '060830026': 4,
    '060830027': 3, '060830028': 3,
}


def geocode_to_tract(address: str):
    """Resolve a free-text address to a Census GEOID using the Census Geocoder."""
...
def lookup_district(tract_id: str) -> int:
    """Return the supervisorial district for a tract."""
    for ejc_name, ejc in SB_EJ_COMMUNITIES.items():
        if tract_id in ejc['tracts']:
            return ejc['district']
    prefix = tract_id[:9]
    return TRACT_TO_DISTRICT.get(prefix, 1)
...
app_mode = st.radio(
    "Dashboard Mode",
    ["Command Center", "SB 1000 Audit", "Resident Portal"],
    horizontal=True,
    label_visibility="collapsed",
)

# ============================================================================
# SHARED DATA LOADING (used by all modes)
# ============================================================================
...
has_real_census = REAL_DATA_AVAILABLE and n_tracts > 16

water_levels = get_noaa_water_levels()
has_noaa_data = not water_levels.empty if isinstance(water_levels, pd.DataFrame) and not water_levels.empty else False
...
# ============================================================================
# MODE 2: SB 1000 COMPLIANCE AUDITOR
# ============================================================================

elif app_mode == "SB 1000 Audit":

    st.markdown('<p class="section-title">SB 1000 Environmental Justice Compliance Audit</p>', unsafe_allow_html=True)
    st.markdown(
        '<p style="color: var(--text-secondary); font-size: 0.9rem; margin-top: -0.5rem;">'
        'Auditing 8 official EJ Communities against the Santa Barbara County 2030 Climate Action Plan. '
        'Target: March 10, 2026 Board of Supervisors EJ Element vote.</p>',
        unsafe_allow_html=True,
    )
    ...
    with audit_col1:
        audit_severity = st.slider("Storm Severity", 0.0, 1.0, 0.5, 0.05, key="audit_sev")
    with audit_col2:
        audit_duration = st.selectbox("Duration", [7, 14, 21, 30], index=2, format_func=lambda x: f"{x} days", key="audit_dur")
    with audit_col3:
        audit_start = st.selectbox("Shock Start", [15, 30, 45, 60], index=1, format_func=lambda x: f"Day {x}", key="audit_start")

    # --- Run per-EJC simulations ---
    county_avg_recovery = 0.0
    ejc_rows = []
    all_ej_pcts = []

    for ej_name, ej_info in SB_EJ_COMMUNITIES.items():
        tract_ids = ej_info['tracts']
        matching = sb_tracts[sb_tracts['tract_id'].isin(tract_ids)]
        ...
        if matching.empty:
            ...
            ej_pct = int(matching['ej_percentile'].mean())
            pop = int(matching['population'].sum())
        ...
        ejc_rows.append({
            'Community': ej_name,
            'District': ej_info['district'],
            'Supervisor': SUPERVISORS[ej_info['district']],
            'EJ Percentile': ej_pct,
            'Population': pop,
            'Coastal Jobs %': coastal_pct,
            'Recovery Time (days)': round(rec, 1),
            'Beta': round(beta, 6),
            'lat': ej_info['lat'],
            'lon': ej_info['lon'],
        })
        all_ej_pcts.append(ej_pct)
    ...
    ejc_df['County Avg (days)'] = round(county_avg_recovery, 1)
    ejc_df['Gap %'] = ((ejc_df['Recovery Time (days)'] - county_avg_recovery) / county_avg_recovery * 100).round(1)
    ejc_df['Compliance'] = ejc_df['Gap %'].apply(lambda g: 'HIGH-PRIORITY RISK' if g > 20 else 'Compliant')

    flagged_count = (ejc_df['Compliance'] == 'HIGH-PRIORITY RISK').sum()

    # --- Summary card ---
    st.markdown(f"""
    <div class="glass-card" style="border-color: {'var(--danger)' if flagged_count > 0 else 'var(--success)'}; padding: 1.25rem 1.5rem;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                <div style="font-size: 1.5rem; font-weight: 700;">{flagged_count} of 8 EJCs flagged as High-Priority Compliance Risk</div>
                <div style="color: var(--text-secondary); font-size: 0.875rem; margin-top: 0.25rem;">
                    Policy Gap Threshold: &gt;20% above county average recovery time &bull; County Avg: {county_avg_recovery:.0f} days
                </div>
            </div>
            <div style="font-size: 3rem; font-weight: 800; font-family: 'JetBrains Mono'; color: {'var(--danger)' if flagged_count > 3 else 'var(--warning)'};">{flagged_count}/8</div>
        </div>
    </div>
    """, unsafe_allow_html=True)

    # --- Audit table ---
    st.markdown('<p class="section-title" style="margin-top: 1.5rem;">EJ Community Compliance Table</p>', unsafe_allow_html=True)

    display_df = ejc_df[['Community', 'District', 'Supervisor', 'EJ Percentile',
                          'Recovery Time (days)', 'County Avg (days)', 'Gap %', 'Compliance']].copy()
    st.dataframe(
        display_df.style.applymap(
            lambda v: 'color: #ef4444; font-weight: 600' if v == 'HIGH-PRIORITY RISK' else 'color: #22c55e',
            subset=['Compliance']
        ),
    ...
    # --- EJC Map ---
    col_audit_map, col_audit_detail = st.columns([2, 1])

    with col_audit_map:
        st.markdown('<p class="section-title">EJ Community Map</p>', unsafe_allow_html=True)
        ejc_df['color_val'] = ejc_df['Gap %'].clip(-10, 60)
        fig_audit = px.scatter_mapbox(
            ejc_df, lat='lat', lon='lon', color='color_val',
            size='Population', hover_name='Community',
            hover_data={'District': True, 'Supervisor': True, 'Recovery Time (days)': True,
                        'Gap %': True, 'Compliance': True, 'lat': False, 'lon': False, 'color_val': False},
            color_continuous_scale=[[0, '#22c55e'], [0.4, '#eab308'], [1, '#ef4444']],
            range_color=[-10, 60], size_max=30, zoom=8,
            center={'lat': 34.7, 'lon': -120.0}, mapbox_style='carto-darkmatter',
            labels={'color_val': 'Gap %'},
        )
        fig_audit.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=400,
                                paper_bgcolor='rgba(0,0,0,0)', font=dict(family="Inter", color='#a0a0b0'))
        st.plotly_chart(fig_audit, use_container_width=True, config={'displayModeBar': False})

    with col_audit_detail:
        st.markdown('<p class="section-title">CAP CP-1 Green Re-Skilling Optimizer</p>', unsafe_allow_html=True)

        selected_ejc = st.selectbox("Select EJ Community", list(SB_EJ_COMMUNITIES.keys()), key="audit_ejc_select")
        ejc_row = ejc_df[ejc_df['Community'] == selected_ejc].iloc[0]

        sensitive_count = int(ejc_row['Population'] * ejc_row['Coastal Jobs %'] / 100)
        if sensitive_count < 10:
            sensitive_count = 200

        opt = optimize_policy_allocation(
            current_beta=ejc_row['Beta'],
            ej_percentile=ejc_row['EJ Percentile'],
            sensitive_worker_count=sensitive_count,
            shock_severity=audit_severity,
            shock_duration=float(audit_duration),
            shock_start=float(audit_start),
        )

        st.markdown(f"""
        <div class="glass-card">
            <div class="metric-label">Optimized Intervention — {selected_ejc}</div>
            <div style="margin-top: 1rem;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.6rem;">
                    <span style="color: #a0a0b0;">Workers to Re-skill</span>
                    <span style="font-family: 'JetBrains Mono'; font-weight: 600;">{opt['workers_to_reskill']:,}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.6rem;">
                    <span style="color: #a0a0b0;">Budget Required</span>
                    <span style="font-family: 'JetBrains Mono'; font-weight: 600; color: var(--accent-blue);">${opt['total_budget']:,.0f}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.6rem;">
                    <span style="color: #a0a0b0;">Beta Reduction</span>
                    <span style="font-family: 'JetBrains Mono'; font-weight: 600; color: var(--success);">-{opt['beta_reduction_pct']:.1f}%</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.6rem;">
                    <span style="color: #a0a0b0;">Recovery Improvement</span>
                    <span style="font-family: 'JetBrains Mono'; font-weight: 600; color: var(--success);">{opt['recovery_improvement_days']:.0f} days faster</span>
                </div>
                <div style="display: flex; justify-content: space-between;">
                    <span style="color: #a0a0b0;">Optimal Re-skill %</span>
                    <span style="font-family: 'JetBrains Mono'; font-weight: 600;">{opt['optimal_reskill_pct']*100:.1f}%</span>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)

        # --- Generate Board Letter button ---
        if st.button("Generate Board Agenda Letter", key="gen_letter"):
            letter = generate_board_agenda_letter(
                tract_name=selected_ejc,
                supervisor_name=ejc_row['Supervisor'],
                district=int(ejc_row['District']),
                optimized_budget=opt['total_budget'],
                workers_to_reskill=opt['workers_to_reskill'],
                beta_before=opt['beta_before'],
                beta_after=opt['beta_after'],
                recovery_time_before=opt['recovery_time_before'],
                recovery_time_after=opt['recovery_time_after'],
                ej_percentile=ejc_row['EJ Percentile'],
            )
            ...
            st.download_button(
                "Download Letter (.txt)",
                data=letter,
                file_name=f"board_letter_{selected_ejc.replace(' ', '_').lower()}.txt",
                mime="text/plain",
                key="dl_letter",
            )

# ============================================================================
# MODE 3: HYPER-LOCAL RESIDENT PORTAL
# ============================================================================

elif app_mode == "Resident Portal":

    st.markdown('<p class="section-title">Resident Portal</p>', unsafe_allow_html=True)
    st.markdown(
        '<p style="color: var(--text-secondary); font-size: 0.95rem; margin-top: -0.5rem;">'
        'Enter your Santa Barbara County address to see personalized risk data and contact your County Supervisor about the March 10, 2026 EJ Element vote.</p>',
        unsafe_allow_html=True,
    )

    address_input = st.text_input(
        "Your Address",
        placeholder="e.g. 735 State St, Santa Barbara, CA 93101",
        key="resident_address",
    )

    if address_input:
        with st.spinner("Looking up your Census tract..."):
            tract_geoid = geocode_to_tract(address_input)

        if tract_geoid is None:
            st.warning("Could not locate that address. Try including the full city and ZIP code.")
        else:
            matching_tract = sb_tracts[sb_tracts['tract_id'] == tract_geoid]
            if matching_tract.empty:
                st.warning(f"Tract {tract_geoid} found but not in our Santa Barbara dataset. It may be outside the county.")
            else:
                t = matching_tract.iloc[0]
                district = lookup_district(tract_geoid)
                supervisor = SUPERVISORS.get(district, "Unknown")
                ...
                L_vals = sol_r['L']
                dl_dt_peak = min(
                    (L_vals[i+1] - L_vals[i]) / ((sol_r['t'][i+1] - sol_r['t'][i]) if (sol_r['t'][i+1] - sol_r['t'][i]) > 0 else 1)
                    for i in range(len(L_vals) - 1)
                )

                # --- Personalized risk card ---
                st.markdown(f"""
                <div class="glass-card" style="border-color: var(--danger); margin-top: 1rem;">
                    <div style="font-size: 1.25rem; font-weight: 700; margin-bottom: 0.5rem;">
                        Your Neighborhood's Risk in the March 10th Vote
                    </div>
                    <div style="font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 1rem;">
                        {t['name']} &bull; Census Tract {tract_geoid} &bull; District {district}
                    </div>
                </div>
                """, unsafe_allow_html=True)

                col_risk1, col_risk2, col_risk3, col_risk4 = st.columns(4)
                col_risk1.metric("Recovery Time", f"{rec_time_r:.0f} days")
                col_risk2.metric("EJ Burden", f"{t['ej_percentile']}th pctile")
                col_risk3.metric("Exodus Prob.", f"{exodus_r:.0%}")
                col_risk4.metric("Your Supervisor", f"D{district}: {supervisor.split('(')[0].strip()}")
                ...
                st.markdown(f"""
                <div class="glass-card">
                    <div class="metric-label">Your County Supervisor</div>
                    <div style="font-size: 1.5rem; font-weight: 700; margin-top: 0.5rem;">{supervisor}</div>
                    <div style="color: var(--text-secondary); font-size: 0.875rem;">Supervisorial District {district}</div>
                </div>
                """, unsafe_allow_html=True)

                # --- 1-Click Public Comment ---
                st.markdown('<p class="section-title" style="margin-top: 1.5rem;">Evidence-Based Public Comment</p>', unsafe_allow_html=True)
                st.markdown(
                    '<p style="color: var(--text-muted); font-size: 0.8rem;">'
                    'This comment is auto-generated from your tract data. Edit as needed, then click the email link to send.</p>',
                    unsafe_allow_html=True,
                )

                housing_pressure_val = 0.0
                if HOUSING_DATA_AVAILABLE:
                    try:
                        h_data = load_housing_data('APR_Download_2024.xlsx')
                        h_pressure = h_data['pressure']
                        housing_pressure_val = float(h_pressure['housing_pressure_index'].mean())
                    except Exception:
                        pass

                comment = generate_public_comment(
                    tract_name=t['name'],
                    supervisor_name=supervisor,
                    district=district,
                    recovery_time=rec_time_r,
                    ej_percentile=t['ej_percentile'],
                    exodus_prob=exodus_r,
                    dl_dt=dl_dt_peak,
                    housing_pressure=housing_pressure_val,
                )

                edited_comment = st.text_area("Your Public Comment", value=comment, height=250, key="comment_area")

                import urllib.parse
                subject = urllib.parse.quote(f"Public Comment: EJ Element Vote — {t['name']}")
                body = urllib.parse.quote(edited_comment)
                mailto_link = f"mailto:?subject={subject}&body={body}"

                st.markdown(f"""
                <a href="{mailto_link}" class="export-btn" style="display: inline-block; margin-top: 0.5rem; text-decoration: none;">
                    Email This Comment to Supervisor {supervisor.split('(')[0].strip()}
                </a>
                """, unsafe_allow_html=True)
                ...
                st.download_button(
                    "Download as Text",
                    data=edited_comment,
                    file_name="public_comment_ej_element.txt",
                    mime="text/plain",
                    key="dl_comment",
                )
